...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const{data:org}=await admin.from("organizations").select("timezone,license_status,paid_until").eq("id",employee.organization_id).single();if(org?.license_status!=="active"||(org.paid_until&&new Date(org.paid_until+"T23:59:59")<new Date()))return json({error:"Licença da empresa pendente ou vencida."},402);const localDate=(v:Date)=>new Intl.DateTimeFormat("en-CA",{timeZone:org?.timezone??"America/Sao_Paulo",year:"numeric",month:"2-digit",day:"2-digit"}).format(v);const today=localDate(new Date());
